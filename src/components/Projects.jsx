...
const projects = [
  {
    ...
    status: 'Selesai',
    statusClass: 'bg-green/15 text-green border border-green/30',
  ...
  },
  {
    ...
    status: 'Fail',
    statusClass: 'bg-red/15 text-red border border-red/30',
  ...
  return (
    <section id="projects" className="py-24 px-[5%] bg-transparent">
      <div ref={ref} className="max-w-[900px] mx-auto reveal">
        ...
        <div className="font-display text-[clamp(28px,4vw,36px)] font-bold text-white leading-[1.2] tracking-tight mb-4">Featured Projects</div>
        ...
        <div className="grid gap-5">
          {projects.map((p) => (
            <div key={p.name} className="bg-white/[0.07] backdrop-blur-2xl border border-white/[0.10] rounded-3xl p-7 shadow-glass transition-all duration-300 hover:bg-white/[0.12] hover:-translate-y-[4px] hover:shadow-xl hover:shadow-black/30">
              <div className="flex justify-between items-start mb-3">
                <div className="font-display font-bold text-lg text-white tracking-tight">{p.name}</div>
                <span className={`text-[11px] font-semibold px-[10px] py-[3px] rounded-full ${p.statusClass}`}>
                  {p.status}
                </span>
              </div>
              <p className="text-sm text-white/55 mb-4 leading-[1.65]">{p.desc}</p>
              <ul className="list-none mb-5 flex flex-col gap-[6px]">
                {p.features.map((f) => (
                  <li key={f} className="text-[13px] text-white/55 flex gap-2">
